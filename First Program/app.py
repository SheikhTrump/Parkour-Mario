...
class GameState:
    ...
    def move_to_next_tile(self):
        if self.current_tile < len(self.tiles) - 1:
            ...
            # Show final round message if this is the second-to-last tile
...
def draw_crosshair():
    ...
    # Disable lighting for crosshair
    glDisable(GL_LIGHTING)

    # Draw crosshair (simple plus sign)
...
def display():
    ...
    if game_state.showing_difficulty_menu:
        ...
        # Draw crosshair (only when not showing other messages)
